<template>
  <header class="header" :style='[main ? {"background": "transparent"} : {"background": "#ffffff"}]'>
    ...
    <div class="menu">
      ...
      <ul v-if="show" :style='[main ? {"background": "#0D87AC"} : {"background": "#ffffff"}]'>
        <div >
          <router-link v-for="i in items" :style='[main ? {"color": "#ffffff"} : {"color": "#0098C7"}]' :key="i.id" :to="i.path">{{ i.name }}</router-link>
        </div>
      </ul>
    </div>
  </header>
